...
test <- foreach(i = rev(1:nrow(sp_list)),
                ...
                .errorhandling = "pass") %dopar%
  {
    ...
    if(file.exists(paste0("Indices/Ind_latlong_",aou,".rds")) &
       (!file.exists(paste0("Trends/",aou,"_latlong_trends.rds")) | re_run)){
      ...
      inds <- readRDS(paste0("Indices/Ind_latlong_",aou,".rds"))
...
# Estimate trends for long- short- and three-gen --------------------------
      ...
      trends_out <- trends_out  %>%
        mutate(precision = reliab_func_prec(width_of_95_percent_credible_interval_slope),
      ...
      saveRDS(trends_out, file = paste0("Trends/",aou,"_latlong_trends.rds"))

      saveRDS(inds_out, file = paste0("Indices/list_",aou,"_latlong_indices.rds"))


    saveRDS(maps_out,file = paste0("Figures/temp_rds_storage/",aou,"_latlong_maps.RDS"))
