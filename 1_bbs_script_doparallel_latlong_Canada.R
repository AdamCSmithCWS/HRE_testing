...
prov_state <- load_map("prov_state")  %>%
  ...
  summarise() %>%
  rename(province = strata_name)

lat_long_canada <- load_map("latlong") %>%
  st_join(.,canada,
          left = FALSE) %>%
  st_intersection(.,countries) %>%
  mutate(strata_name = paste(country_code,strata_name,sep = "_")) %>%
  group_by(strata_name,country_code) %>%
  summarise() %>%
  st_union(.,by_feature = TRUE) %>%
  ungroup()
  # %>%
  # st_make_valid() %>%
  # assign_prov_state(.,min_overlap = 0.6)
  # mutate(province = ifelse(country == "Canada",
  #                          near_canada,
  #                          "US")) %>%
  # select(-near_canada)

tst <- ggplot()+
  geom_sf(data = lat_long_canada,
...
# identifying first years for selected species ----------------------------
   ...
   strat <- "latlong_canada"

   s <- stratify(by = strat,
                 strata_custom = lat_long_canada,
              species = sp,
              quiet = TRUE) %>%
